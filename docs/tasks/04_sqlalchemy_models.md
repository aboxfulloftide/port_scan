# Task 04 — Shared SQLAlchemy Models & DB Session

## Status: COMPLETE ✅

**Completed:** 2026-02-22
**Files created:** `shared/__init__.py`, `shared/db.py`, `shared/models.py`
**Deviations from plan:**
- `db.py` reads `DATABASE_URL` from `.env` and replaces `pymysql` with `aiomysql` at runtime (not separate env vars per component)
- All 13 ORM models defined and match the MySQL schema exactly
...
## Objective
Build the shared database layer used by both the API and the worker. This includes the async SQLAlchemy engine, session factory, and all ORM model classes mapped to the MySQL tables.

---

## Files to Create

### `/home/matheau/code/port_scan/shared/db.py`
```python
import os
...
### `/home/matheau/code/port_scan/shared/models.py`
```python
...
### `/home/matheau/code/port_scan/shared/__init__.py`
```python
# shared package
```
...
## Acceptance Criteria
- [ ] `shared/db.py` creates async engine without errors
- [ ] `shared/models.py` imports cleanly with no SQLAlchemy warnings
- [ ] All 13 ORM models defined and match the MySQL schema exactly
- [ ] `get_db()` dependency yields a working async session
- [ ] Test connection: `python -c "from shared.db import engine; print('OK')"`
